...
constraint M_member_detail_fk foreign key (mem_num) 
...
create table M_board(
  ...
  constraint M_board_M_member_fk foreign key (mem_num)
                                references M_member (mem_num),
  constraint M_board_M_category_fk foreign key (cate_num)
                                references M_category (cate_num)
);
...
create table M_category(
  ...
  constraint M_category_M_board_fk foreign key (board_num)
...
create table M_reserve(
  ...
  time_num number not null,
  mem_num number not null,
  reserve_seat varchar2 not null,
  reserve_patment number(1), not null, /*결제수단 (1:카드결제/2:현금결제/3:카카오페이)*/
  reserve_paytotal number not null,
  
  constraint M_reserve_pk primary key (revseat_num),
  constraint M_reserve_M_time_fk foreign key (time_num) 
                                references M_time (time_num),
  constraint M_reserve_M_member_fk foreign key (mem_num) 
                                references M_movie (mem_num)
);
...
create table M_review(
  ...
  constraint M_review_M_movie_fk foreign key (movie_num) 
                                references M_movie (movie_num),
  constraint M_review_M_member_fk foreign key (mem_num) 
                                references M_member (mem_num)
);
...
create table M_like(
  ...
  constraint M_like_M_review_fk foreign key (review_num) 
                                references M_review (review_num),
  constraint M_like_M_member_fk foreign key (mem_num) 
                                references M_member (mem_num)
);
