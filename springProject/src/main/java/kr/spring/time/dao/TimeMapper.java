...
public interface TimeMapper {
	...
	@Select("select movie_time, total_count, reserv_count, total_count-reserv_count rest_count FROM (select movie_time, (select count(*) from m_seat where theater_num=#{theater_num}) total_count, count(*) reserv_count from m_reservseat r join m_time using(time_num) where theater_num=#{theater_num} and to_char(movie_date,'YYYY/MM/DD')=#{movie_date} group by movie_time)")
