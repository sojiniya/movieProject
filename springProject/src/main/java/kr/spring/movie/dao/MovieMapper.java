...
public interface MovieMapper {
	...
	//좋아요
	@Select("SELECT COUNT(*) FROM My_movie m WHERE m.movie_num=#{movie_num} AND m.mem_num=#{mem_num}")
	public MovieLikeVO checkLike(MovieLikeVO movielike); //좋아요 여부 체크
	@Insert("INSERT INTO My_movie (my_movie_num,movie_num,mem_num) VALUES (My_movie_seq.nextval,#{movie_num},#{mem_num}")
	public MovieLikeVO insertLike(MovieLikeVO movielike); //좋아요 추가
	@Delete("DELETE FROM My_movie WHERE m.movie_num=#{movie_num} AND m.mem_num=#{mem_num}")
	public MovieLikeVO deleteLike(MovieLikeVO movielike); //좋아요 취소
	@Select("SELECT COUNT(*) FROM My_movie m WHERE m.movie_num=#{movie_num}")
	public MovieLikeVO countLike(MovieLikeVO movielike); //좋아요 갯수
